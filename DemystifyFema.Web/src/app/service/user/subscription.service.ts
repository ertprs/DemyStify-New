...
export class SubscriptionUserService {
    ...
    gotopaytm(req): Observable<any> {
        if (req) {
            let data = this._httpClient.post(Global.Node_API_SITE, req, { responseType: 'text' });
            return data;
        }
    }
    getPaytmOrderStatus(req): Observable<any> {
        if (req) {
            let headers = new HttpHeaders({ 'Content-Type': 'application/json' });
            let data = this._httpClient.post(Global.API_SITE + "common/api/paytmorderstatus", req, { headers: headers });
            return data;
        }
    }
}