...
export class SubscriptionUserComponent implements OnInit {
    ...
    UpdateSubscriptionInDB() {
        this.spinnerService.show();
        var currentDate = this._datePipe.transform(this.currentDate, 'yyyy/MM/dd');
        this.subscriptionupdate.SubscriptionId = this.subscriptions[0].SubscriptionId;
        this.subscriptionupdate.StartDate = currentDate;
        this.subscriptionupdate.PaymentDate = currentDate;
        this._subscriptionAdminService.updateSubscription(this.subscriptionupdate)
            .subscribe(data => {
                this.spinnerService.hide();
                if (data.Status == Global.API_SUCCESS) {                   
                    this.toastr.success(data.Description, Global.TOASTR_SUBSCRIPTION_TITLE, { closeButton: true });
